...
try {
        ...
        if (empty($code) || $discount <= 0 || $discount > 1 || $usage_limit <= 0 || empty($expire_date_input) || empty($expire_time_input)) {
            ...
            if ($stmt->fetch()) {
                ...
                if ($action == 'add') {
                    ...
                    $stmt->execute([$new_id, $code, $discount, $usage_limit, $expire_date]);
                    $success = "Yeni GENEL kupon başarıyla eklendi.";
                    ...
                    $success = "Genel kupon başarıyla güncellendi.";
                }
            }
        }
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['action'])) {
        ...
        if (!$id) {
            ...
            if ($action == 'delete') {
                $stmt = $pdo->prepare(
                    "DELETE FROM Coupons WHERE id = ? AND company_id IS NULL"
                );
                $stmt->execute([$id]);
                $success = "Genel kupon başarıyla silindi.";

            } elseif ($action == 'edit') {
                $form_mode = 'edit';
                $stmt = $pdo->prepare(
                    "SELECT * FROM Coupons WHERE id = ? AND company_id IS NULL"
                );
                $stmt->execute([$id]);
                $coupon_to_edit = $stmt->fetch(PDO::FETCH_ASSOC);

                if (!$coupon_to_edit) {
                    $error = "Düzenlenecek kupon bulunamadı veya bu genel bir kupon değil.";
                    $form_mode = 'add';
                } else {
...
<head>
    ...
    <title>Admin - Genel Kupon Yönetimi</title>
...
<body>
    ...
    <main class="container admin-container">
        ...
        <div class="admin-form-container">
            <h3>
                <?php echo $form_mode == 'edit' ? 'Genel Kuponu Düzenle' : 'Yeni Genel Kupon Ekle'; ?>
            </h3>
            <p>Burada oluşturulan kuponlar tüm firmaların seferlerinde geçerli olacaktır.</p>

            <form action="admin_coupons.php" method="POST" class="admin-form">
                ...
                <?php if ($form_mode == 'edit'): ?>
                    ...
                    <a href="admin_coupons.php" class="button-secondary">İptal</a>
                <?php else: ?>
                    ...
                    <?php if (count($coupons) > 0): ?>
                        <?php foreach ($coupons as $coupon): ?>
                            <tr>
                                ...
                                <td class="actions">
                                    <a href="admin_coupons.php?action=edit&id=<?php echo $coupon['id']; ?>" class="action-button edit">Düzenle</a>
                                    <a href="admin_coupons.php?action=delete&id=<?php echo $coupon['id']; ?>"
                                       class="action-button delete"
                                       onclick="return confirm('Bu genel kuponu silmek istediğinizden emin misiniz?');">Sil</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="5">Kayıtlı genel (tüm firmalarda geçerli) kupon bulunamadı.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
